...
#from /home/user/miniconda3/envs/python37/lib/python3.7/site-packages/eleanor/mast.py import tic_from_coords
...
done_id = []
done_sec = []
with open('/home/user/targets_ran.txt','r') as file:
...
for j,i in enumerate(mass_id[beg_ind:end_ind], start=beg_ind):
    ...
    ###first try to find 2-min data for this sector
                try:
                    print(j, sector)
                    data = fits.open('/data/storage/GALAH/lightkurve/{}_s{}_120s.fits'.format(TIC,sector))
                    source='lightkurve'
                    ending = '120s'
                    ...
                    try:
                        data = fits.open('/data/storage/GALAH/lightkurve/{}_s{}_20s.fits'.format(TIC,sector))
                        source = 'lightkurve'
                        ending = '20s'
                    except Exception as e:
                        #print('Data does not already exist @ 20-s cadence with Lightkurve')
                        try:
                            data = fits.open('/data/storage/GALAH/eleanor/{}_s{}_ffi.fits'.format(TIC,sector)) #have to change for katana storage
                            source='eleanor'
                            ending = 'ffi'
                            ...
                            try:
                                ...
                                lc.download(download_dir='/data/storage/GALAH/lightkurve/')
                                os.rename('/data/storage/GALAH/lightkurve/mastDownload/TESS/{}/{}'.format(twomin_file[0:-8],twomin_file),'/data/storage/GALAH/lightkurve/{}_s{}_120s.fits'.format(TIC,sector))
                                source = 'lightkurve'
                                ending = '120s'
                                ...
                                try:
                                    ...
                                    lc.download(download_dir='/data/storage/GALAH/lightkurve/')
                                    os.rename('/data/storage/GALAH/lightkurve/mastDownload/TESS/{}/{}'.format(twentys_file[0:-8],twentys_file), '/data/storage/GALAH/lightkurve/{}_s{}_20s.fits'.format(TIC,sector))
                                    source = 'lightkurve'	
                                    ending = '20s'
                                except Exception as e:
                                    #print(e)
                                    ...
                                    try:
                                        ...
                                        star = eleanor.Source(coords=coords, sector=int(sector), post_dir='/data/storage/postcards/')
                                        ...
                                        datum.save(output_fn='{}_s{}_ffi.fits'.format(TIC,sector),directory='/data/storage/GALAH/eleanor/')
                                        source = 'eleanor'
                                        ending = 'ffi'
                ...
                try:                
                    if os.path.isfile('/data/storage/GALAH/{}/{}_s{}_{}.fits'.format(source,TIC,sector,ending)):    
                        #os.system('rclone copy /data/storage/GALAH/{}/{}_s{}.fits CloudStor:/{}/'.format(source,TIC,sector,source))
                        totalfile = open('/home/user/targets_ran.txt',"a")

...
tess_start_tjd = []
tess_end_tjd = []
with open('/home/user/documents/orbit_times_20201013_1338.txt', 'r') as file:
	reader = csv.reader(file,delimiter='\t')
	for row in reader:
...
for i, fields in enumerate(simult_field):
	...
	for num in np.arange(1,5):
		for exp in exp_range:
			...
			fits_file = glob.glob('/data/storage/GALAH/{}/data/ccd_{}/*{}.fits'.format(date,num,new_exp))
			if not fits_file:
...
for i, fields in enumerate(simult_field):
	...
	try:
		try:
			...
			with open('/data/storage/GALAH/{}/fld/{}.fld'.format(date,field_fld), 'r') as file:
				reader = csv.reader(file,delimiter=' ')
				next(reader)
				next(reader)
				next(reader)
...
with open('/home/user/documents/simult_obs_targets.txt', 'w') as file:

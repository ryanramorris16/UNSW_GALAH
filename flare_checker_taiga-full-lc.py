...
for index, target in enumerate(good_shot_flares):
	try:
		...
		images_num = np.arange(0, len(lc.time), 1)
		med_images = []
		sub_images = []
		...
		std_for_plot = np.std(lc.time[mid_time_ind-576:mid_time_ind+144]) #change to [0:-1] if want std of whole plot
		...
		im3 = ax3.set_xlim(left=lc.time[0], right=lc.time[-1])
		...
		anim.save('/data/storage/GALAH/Simult_Gifs/Feb9/{}_std2.mp4'.format(target))
		plt.close('all')

...
with open('/home/user/documents/simult_obs_targets.txt', 'r') as file:
	...
	if target_sector[index] <= 31:
		...
		try:
			...
			try:
				...
				data = fits.open('/data/storage/GALAH/all_target_lc/{}_s{}_eleanor_data.fits'.format(ids,target_sector[index]))
				...
				try:
					...
					data.save(output_fn='{}_s{}_eleanor_data.fits'.format(ids,target_sector[index]),directory='/data/storage/GALAH/all_target_lc/')
					data_source.append('30-min')
		...
		try:
			...
			ax1.axvspan(target_start_mjd[index],target_end_mjd[index], color='b', alpha=0.5)
			ax2.scatter(lc.time, lc.flux, c='k', marker='.')
			ax2.axvspan(target_start_mjd[index],target_end_mjd[index], color='b', alpha=0.5)
			ax2.set_xlim(left=target_start_mjd[index]-1,right=target_end_mjd[index]+1)
			ax1.set_xlabel('Time')
			ax1.set_ylabel('Normalized Flux')
			plt.savefig('/data/storage/GALAH/Simult_Plots_Ratios/Bin_{}/{}/{}_lc_sector_{}_{}.png'.format(binner,data_source[index], ids, target_sector[index], data_source[index]))

			print("Lightcurve successfully plotted, and saved in Simult_Plots_Ratios")

			with open('/data/storage/GALAH/Simult_Plots_Ratios/Bin_{}/{}_sector_{}.txt'.format(binner,ids,target_sector[index]), 'w') as file:

...
exo_per = []
tls_per = []
with open('/home/user/documents/tlst_bkg_period_small_sigma.txt', 'r') as file:
...
tls_per = []
tls_error = []
with open('/home/user/documents/spoc_tls_periods.txt', 'r') as file:
	reader = csv.reader(file,delimiter='\t')
	for row in reader:
		exo_per.append(float(row[0]))
...
for i, j in enumerate(bad_targets):
	if bad_tls[i] != 0:
		...
		plt.savefig('/home/user/documents/Faint_Bad_Sigma_QLP_Plots/{}_folded_lc.png'.format(bad_targets[i]))

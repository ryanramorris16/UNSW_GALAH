...
class target:
    ...
    def __init__(self, date, oversample_factor, beginning):#, end): #dir, ccd, date, exp, fiber, oversample_factor, coadd=None):    
        ...
        galah_obs_dec = []
        galah_exp = []
        with open('/home/user/documents/fieldlogs_export.txt', 'r') as file:
        ...
        com_files = os.listdir('/data/storage/GALAH/Simult_GALAH_Data/reductions/results/{}/spectra/com/'.format(self.date))
        com_dict = {"test gaia": "test file"}
        for file_name in com_files:
            com = fits.open('/data/storage/GALAH/Simult_GALAH_Data/reductions/results/{}/spectra/com/{}'.format(self.date, file_name))  
            gaia = com[1].header['GAIA_ID']
            com_dict['{}'.format(gaia)] = file_name
            com.close()
        
        fits_files = os.listdir('/data/storage/GALAH/Simult_GALAH_Data/reductions/results/{}/spectra/all/'.format(self.date))
        ...
        for file_name in ew_files[self.beg_ind::]:#self.end_ind]:            
            #full_path = os.path('/data/storage/GALAH/Simult_GALAH_Data/reductions/results/{}/spectra/all/{}'.format(self.date, file_name))
            print(file_name)
            ccd_proxy = file_name[-6]       #should be last digit of file before .fits
            hdu = fits.open('/data/storage/GALAH/Simult_GALAH_Data/reductions/results/{}/spectra/all/{}'.format(self.date, file_name), memmap=False)
            gaia = hdu[1].header['GAIA_ID']
            com = fits.open('/data/storage/GALAH/Simult_GALAH_Data/reductions/results/{}/spectra/com/{}'.format(self.date, com_dict['{}'.format(gaia)]), memmap=False)          
    ...
    def equivalent_width(self):
        ...
        for index in range(len(self.flux)):
            ...
            if int(ccd) == 1 or int(ccd) == 3:
                ...
                plt.savefig('/data/storage/GALAH/EW_Plots/{}.png'.format(self.file_name[index][0:-5]))
                plt.close()
                '''
            else:
                ew = 0
    ...
    def writeout(self):
        file = open('/home/user/documents/spectra_ew_all.txt', "a")
        ...
        #with open('/home/user/documents/spectra_ew_{}-{}.txt'.format(self.beg_ind, self.end_ind), 'w') as file:
            #for number, values in enumerate(self.gaia_ids[self.beg_ind:self.end_ind]):
                #file.write(str(self.gaia_ids[number])+'\t'+str(self.file_name[number])+'\t'+str(self.equivwidth[number])+'\t'+str(self.equiverror[number])+'\t'+str(self.medians[number])+'\n')
